...
public interface reports_templateRepository extends CrudRepository<reports_template, Integer> {
	...
	@Transactional
	@Modifying
	@Query("update reports_template set CellValue = ?3 where ReportName = 'CR9' and RowNumber = ?1 and ColumnNumber = ?2")
	void updateCR9(Integer row, Integer col, String value);

	@Transactional
	@Modifying
	@Query("update reports_template set CellValue = ?3 where ReportName = 'CR4' and RowNumber = ?1 and ColumnNumber = ?2")
	void updateCR4(Integer row, Integer col, String value);
